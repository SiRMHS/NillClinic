...
Kept out of the app on purpose: this is a one-time backfill of a spreadsheet
that no longer receives writes, so it does not justify an xlsx parser in the
API's dependency tree.
"""
import json, re, sys
import openpyxl
...
def find_header(ws, marker, limit=10):
    ...
    for i, row in enumerate(ws.iter_rows(min_row=1, max_row=limit, values_only=True), start=1):
        ...
        if any(marker in l for l in labels if l):
            return i, labels
    raise SystemExit(f"header not found in {ws.title!r} (looking for {marker!r})")
...
def main(path, out):
    ...
    followup_sheets = [
        "تماس های فالوآپ اردیبهشت", "تماس های فالوآپ مرداد", "تماس های فالوآپ خرداد ",
        "تماس های فالواپ تیر", "تماس های فالوآپ بهمن",
    ]
    ...
    for sheet_name in ("نظرسنجی", "نظرسنجی مرداد "):
        ws = wb[sheet_name]
    ...
    # ── مراجعین رنوویون ──
    ws = wb["مراجعین رنوویون "]
    last_contact_date = None
    for r in data_rows(ws, 19):
    ...
    # ── روزهای پزشکان ──
    ws = wb["روزهای پزشکان"]
    header = cells(next(ws.iter_rows(max_row=1, values_only=True)), 9)
    doctors = [norm(c) for c in header[1:]]
    schedule = []
    for r in data_rows(ws, 9):
