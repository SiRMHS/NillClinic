...
* اگر داشبورد و API روی دو هاستِ یک دامنه باشند (`dash.example.com` و
  `api.example.com`)، متغیر `COOKIE_DOMAIN=.example.com` را روی API ست کنید.
  بدون آن کوکی `jc_csrf` فقط برای هاست API معتبر است: مرورگر آن را می‌فرستد ولی
  صفحه‌ی داشبورد نمی‌تواند بخواندش و هر ذخیره‌ای ۴۰۳ می‌گیرد. فقط توکن CSRF
  گسترده می‌شود؛ کوکی نشست همچنان host-only روی هاست API می‌ماند.
