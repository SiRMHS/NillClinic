...
**نرمالایز نام پزشک:** شیت‌ها با دست پر می‌شوند، پس یک نفر با ۶ املا نوشته شده
(«طناز فخیم»، «فخیم»، «طنازفخیم»، «طنا زفخیم»، «فحیم»، «طناز فخمی») — ۶۲ املا برای
حدود ۱۲ نفر. `scripts/doctor_names.py` هرکدام را به نام رسمی همان شخص در
`reception_items.personnel_name` می‌رساند (فهرست در `scripts/doctor-names.txt`)، تا
سطرهای واردشده با گزارش‌ها یک‌کاسه شوند نه اینکه به ۶ «پزشک» تکه شوند. چیزی حدس زده
نمی‌شود: نامی که به دو نفر بخورد («لطفی» هم الهه است هم شیوا) دست‌نخورده می‌ماند، و
سلولی که نام خدمت است («کاشت مو»، «ویزیت») خالی می‌شود. تست: `python3 -m unittest
